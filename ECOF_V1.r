...
# o1<-tls.A03(x@Y,x@X,x@noise1,x@noise2,nsig=5,df2=220,REG=TRUE)
  ...
  if(REG) C1<-Creg(noise1) else C1<-t(noise1)%*%noise1/nn1  ##计算噪音的规则化，或者直接协方差
  x<-X
...
# P is pre-whitening operator
  ...
  for(ith in (nx+1):n){
    ...
    nd<-length(u$d)
    ui<-t(u$u[,nd])
    n2t<-noise2%*%t(P[1:ith,])  ###对noise进行pre-whitening 
    r1.stat<-d[nd]/((ui%*%t(n2t)%*%n2t%*%t(ui))/nn2)  ####
    dhat<-rep(NA,nx+1)
    for(i in 1:(nx+1)){
      vi<-t(u$u[,i])
      dhat[i]<-d[i]/(vi%*%t(n2t)%*%n2t%*%t(vi)/nn2) ###27 in AS03
    }
...
# then get max/min value as upper/lower bound for scaling factor beta_hat
    ...
    ai<-unit*sqrt(Crit)
    bi<-cbind(ai,NA)
    for(i in 1:nx) bi[,i]<-ai[,i]/sqrt(delta.dhat[i])  ####31 in A03
    bi[,nx+1]<-apply(t(t(bi[,1:nx])),1,function(x) {sqrt(1-sum(x^2))})  ###the weight on eigenvectors
    nsim<-dim(bi)[1]
...
ols<-function(Y,X,noise1,noise2,nsig,df2=NULL,plev=.9){
# sample command lines:
# x<-readin('obs_sig.txt','noise1.txt','noise2.txt')
