...
def open_ti1_year(year: int) -> xr.DataArray:
    ...
    # 现在严格对齐（应该都 365 天了）
    vws = (vws_da).astype("float32").transpose("time", "plev", "lat", "lon")
    time3 = time.time()
    time3_2 = time3-time2
...
thr_plev = xr.open_dataset("/home/dongyl/Work2_2025/ERA5_daily/era5_VWS_threshold_95p_plev.nc")["vws_threshold"]
for year in YEARS:
    vws = open_ti1_year(year)
    print("VWS done")
    exceed = vws > thr_plev
    ...
    out = f"{OUT_DIR}/era5_VWS_exceed_pct_{year}_{int(Q * 100)}p.nc"
