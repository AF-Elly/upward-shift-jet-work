...
# 示例并行计算函数调用
def parallel_turbulence_analysis(index_hist, index_future, lat, num_chunks=64):

    turbulence_probability_list = []
    hist_threshold_list = []
    for lev in range(index_hist.shape[1]):
        print(lev)
        # 计算第99%分位点的 DEF 阈值
        threshold, cdf = compute_def_threshold_filtered(index_hist[:, lev], lat, num_chunks=num_chunks)
        print('threshold done!')
        # 计算湍流发生的概率
        probability = compute_turbulence_probability(index_future[:, lev], threshold)
        turbulence_probability_list.append(probability)
        hist_threshold_list.append(threshold)

    turbulence_probability_array = np.stack(turbulence_probability_list)
    hist_threshold_array = np.stack(hist_threshold_list)
    return hist_threshold_array, turbulence_probability_array
...
for model in models:
    print(f'{model} begin')
    TI1_hist = np.load(f'/home/dongyl/Work2024/Tl/obs_npy/TI1_historical_{model}_1980-2010_N_new.npy')
    TI1_future = np.load(f'/home/dongyl/Work2024/Tl/obs_npy/TI1_ssp585_{model}_2060-2090_N_new.npy')
    index_hist = TI1_hist
    index_future = TI1_future
    print('ok')
    # 并行计算湍流指数和概率
    hist_threshold_array, turbulence_probability_array = parallel_turbulence_analysis(index_hist, index_future, lat)
    # 保存概率结果
    np.save(f'/home/dongyl/Work2024/Tl/obs_npy/TI1_historical_{model}_1980-2010_N_threshold.npy',
            hist_threshold_array)
    np.save(f'/home/dongyl/Work2024/Tl/obs_npy/TI1_ssp585_{model}_2060-2090_N_500-10hpa_frequency_use_hist_threshold.npy',
            turbulence_probability_array)
    print(f'{model} TI1 threshold and turbulence probability done!')
