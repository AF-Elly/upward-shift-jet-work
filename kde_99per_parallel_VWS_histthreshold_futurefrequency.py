...
for model in models:
    print(f'{model} begin')
    VWS_hist = np.load(f'/home/dongyl/Work2024/Tl/obs_npy/VWS_historical_{model}_1980-2010_N.npy')
    VWS_future = np.load(f'/home/dongyl/Work2024/Tl/obs_npy/VWS_ssp585_{model}_2060-2090_N.npy')
    index_hist = VWS_hist
    index_future = VWS_future
    print('ok')
    # 并行计算湍流指数和概率
    hist_threshold_array, turbulence_probability_array = parallel_turbulence_analysis(index_hist, index_future, lat)
    # 保存概率结果
    np.save(f'/home/dongyl/Work2024/Tl/obs_npy/VWS_historical_{model}_1980-2010_N_threshold.npy',
            hist_threshold_array)
    np.save(f'/home/dongyl/Work2024/Tl/obs_npy/VWS_ssp585_{model}_2060-2090_N_500-10hpa_frequency_use_hist_threshold.npy',
            turbulence_probability_array)
    print(f'{model} VWS threshold and turbulence probability done!')
