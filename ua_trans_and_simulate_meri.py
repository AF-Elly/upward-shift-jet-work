...
plot_meri_section(fig.add_axes(axes[0]),plot_data[0],mean_clim=np.nanmean(T_hist585,0),
                  level_range=np.arange(-10, 10.1, 2),level_clim_slim=np.arange(200,300,10),level_range_clim=np.arange(200,300,10),
...
cbar = plt.colorbar(cc, cax=cbar_ax, orientation='vertical', shrink=0.85, pad=0.05, extend='both')
cbar.set_ticks(level)
cbar.ax.tick_params(axis='both', which='major', direction='in', length=4, labelsize=7)
cbar.set_label(label='$^\circ$C', fontsize=8)
...
cc=plot_meri_section(fig.add_axes(axes[3]),plot_data[3],mean_clim=np.nanmean(ua_hist585,0),
                  level_range=np.arange(-10, 10.1, 2),level_clim_slim=np.arange(-20,40,10),level_range_clim=np.arange(-20,40,10),
...
cbar.set_ticks(level)
cbar.ax.tick_params(axis='both', which='major', direction='in', length=4, labelsize=7)
cbar.set_label(label='m s$^{-1}$', fontsize=8)
